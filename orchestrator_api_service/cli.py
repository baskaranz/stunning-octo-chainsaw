#!/usr/bin/env python3
"""
Command line interface for orchestrator-api-service
"""
import os
import sys
import argparse
...
def run_server(host: str = "0.0.0.0", 
    ...
    if spec is None:
        ...
        uvicorn.run(
            "orchestrator_api_service.app:app", 
            host=host, 
            port=port, 
            reload=reload,
...
def init_project(project_dir: str) -> None:
    ...
    with open(os.path.join(project_dir, "README.md"), "w") as f:
        f.write("""# Orchestrator API Project

This project was created with orchestrator-api-service.

## Running the service

```bash
# Start the service
orchestrator-api --config-path ./config

# For development with auto-reload
orchestrator-api --config-path ./config --reload
```
...
""")
    ...
    logger.info(f"orchestrator-api --config-path {project_dir}/config")

def main():
    """Main entry point for the CLI"""
    parser = argparse.ArgumentParser(description="Orchestrator API Service")
    subparsers = parser.add_subparsers(dest="command", help="Command to run")
    
    # Run command
    run_parser = subparsers.add_parser("run", help="Run the orchestrator API server")
    ...
    if args.command == "run":
        ...
        from orchestrator_api_service import __version__
        print(f"Orchestrator API Service version {__version__}")
