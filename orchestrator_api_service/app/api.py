...
try:
    ...
    if importlib.util.find_spec("app.api.routes"):
    ...
    elif importlib.util.find_spec("orchestrator_api_service.app.routes"):
        # Library app structure
        from orchestrator_api_service.app.routes import router as lib_router
        logger.info("Using routes from library orchestrator_api_service.app.routes")
        ...
        async def root():
            return {
                "message": "Orchestrator API Service is running, but no routes were loaded",
                "status": "warning"
            }
    ...
    async def root():
        return {
            "message": "Orchestrator API Service is running, but could not load routes",
            "status": "error",
            "error": str(e)
        }