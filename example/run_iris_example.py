...
def setup():
    ...
    print(f"   This matches the path in the configuration: sqlite:///example/iris_example.db")
    setup_database()
    ...
    print("   python example/run_iris_example.py --server")
    print("2. Start the orchestrator service (or restart it if already running):")
    print("   python main.py")
    print("3. Test the endpoints:")
    print("   python example/run_iris_example.py --test-orchestrator")
    print("\nTo check if the orchestrator has loaded the iris domain:")
    print("   python example/run_iris_example.py --check-orchestrator")
...
def test_orchestrator_prediction(api_port):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print("   1. Make sure you've run setup: python example/run_iris_example.py --setup")
            print("   2. Restart the orchestrator: python main.py")
            print("   3. Check status: python example/run_iris_example.py --check-orchestrator")
        else:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if 'iris_example' in domains:
                ...
                print("   1. You need to run the setup step first: python example/run_iris_example.py --setup")
...
def show_help():
    ...
    print("       python example/run_iris_example.py --setup")
    print("  2. Run the components:")
    print("     - Start the model server:")
    print("       python example/run_iris_example.py --server [--model-port PORT]")
    ...
    print("       python example/run_iris_example.py --test-direct [--model-port PORT]")
    print("     - Orchestrator endpoint:")
    print("       python example/run_iris_example.py --test-orchestrator [--api-port PORT]")
    print("     - Comparison endpoint:")
    print("       python example/run_iris_example.py --test-comparison [--api-port PORT]")
    print("  4. Check if orchestrator has loaded the iris domain:")
    print("       python example/run_iris_example.py --check-orchestrator [--api-port PORT]")
...
def test_domains_endpoint(api_port):
    ...
    print("\nTesting endpoint debug routes...")
    debug_endpoints = [
        f"http://localhost:{api_port}/debug/endpoint/iris_example/predict",
        f"http://localhost:{api_port}/debug/endpoint/iris_example/predict_local",
        f"http://localhost:{api_port}/debug/endpoint/iris_example/samples"
    ]
    
    for endpoint in debug_endpoints:
        try:
            print(f"\nChecking configuration for: {endpoint}")
            response = requests.get(endpoint)
            print(f"Status: {response.status_code}")
            if response.status_code == 200:
                data = response.json()
                if data.get("config_found", False):
